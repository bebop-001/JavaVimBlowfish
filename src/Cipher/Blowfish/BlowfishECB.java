 ...
 * Downloaded from: http://www.java2s.com/Code/Java/Security/EasyBlowfishencryption.htm
 *
 * Copyright (c) 1997-2002 Markus Hahn <markus_hahn@gmx.net>
 *
 * Released under the Apache 2.0 license.
 *======================================================
 * Sun Mar  3 14:27:35 PST 2019
 * This is the BlowfishECB inner class from the Blowfish.java above.
 */
...
public class BlowfishECB {
    ...
    private final int[] m_sbox1;
    private final int[] m_sbox2;
    private final int[] m_sbox3;
    private final int[] m_sbox4;
    ...
    private long encryptBlock(long lPlainBlock)
    {
        ...
        // encrypt the block, gain more speed by unrolling the loop
        // (we avoid swapping by using nHi and nLo alternating at
        // odd an even loop numbers) and using local references
        ...
        nHi ^= pbox[0];
        nLo ^= (((sbox1[nHi >>> 24] + sbox2[(nHi >>> 16) & 0x0ff])
                ^ sbox3[(nHi >>> 8) & 0x0ff]) + sbox4[nHi & 0x0ff])
                ^ pbox[1];
        nHi ^= (((sbox1[nLo >>> 24] + sbox2[(nLo >>> 16) & 0x0ff])
                ^ sbox3[(nLo >>> 8) & 0x0ff]) + sbox4[nLo & 0x0ff])
                ^ pbox[2];
        nLo ^= (((sbox1[nHi >>> 24] + sbox2[(nHi >>> 16) & 0x0ff])
                ^ sbox3[(nHi >>> 8) & 0x0ff]) + sbox4[nHi & 0x0ff])
                ^ pbox[3];
    ...
    private long decryptBlock(long lCipherBlock) {
        // (same as above)
        ...
        nHi ^= m_pbox[17];
        nLo ^= (((m_sbox1[nHi >>> 24] + m_sbox2[(nHi >>> 16) & 0x0ff])
            ^ m_sbox3[(nHi >>> 8) & 0x0ff]) + m_sbox4[nHi & 0x0ff])
            ^ m_pbox[16];
        nHi ^= (((m_sbox1[nLo >>> 24] + m_sbox2[(nLo >>> 16) & 0x0ff])
            ^ m_sbox3[(nLo >>> 8) & 0x0ff]) + m_sbox4[nLo & 0x0ff])
            ^ m_pbox[15];
        nLo ^= (((m_sbox1[nHi >>> 24] + m_sbox2[(nHi >>> 16) & 0x0ff])
            ^ m_sbox3[(nHi >>> 8) & 0x0ff]) + m_sbox4[nHi & 0x0ff])
            ^ m_pbox[14];
     ...
     * @param outbuffer buffer to get the ciphertext data
     */
    private void encrypt(int[] inbuffer, int[] outbuffer) {
        int nLen = inbuffer.length;
        long lTemp;
    ...
    /**
     * encrypts a long buffer to another long buffer (of the
     * same size or bigger)
     * @param inbuffer buffer with plaintext data
     * @param outbuffer buffer to get the ciphertext data
     */
     ...
     * @param buffer buffer to decrypt
     */
    private void decrypt(int[] buffer)
    {
        int nLen = buffer.length;
        long lTemp;
        for (int nI = 0; nI < nLen; nI +=2)
    ...
    /**
     * decrypts a long buffer to another long buffer (of the
     * same size or bigger)
     * @param inbuffer buffer with ciphertext data
     * @param outbuffer buffer to get the plaintext data
     */
    ...
    // the boxes init. data,
    // FIXME: it might be better to create them at runtime to make
    //        the class file smaller, e.g. by calculating the
    //        hexdigits of pi (default) or just a fixed random
    //        sequence (out of the standard)

    private final static int[] pbox_init = {
        0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822,
        0x299f31d0, 0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377,
        0xbe5466cf, 0x34e90c6c, 0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5,
        0xb5470917, 0x9216d5d9, 0x8979fb1b  };

    private final static int[] sbox_init_1 = {
    ...
    private final static int[] sbox_init_2 = {
    ...
    private final static int[] sbox_init_3 = {
    ...
    private final static int[] sbox_init_4 = {
